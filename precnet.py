...
class PreCNet(Recurrent):
    """
    PreCNet architecture - Straka et al., 2023.
    ...
    # Arguments
        ...
        Ahat_activation: activation function for the prediction (Ahat)
        ...
        output_mode: either 'error', 'prediction', 'all' or layer specification (ex. R2, see below).
            Controls what is outputted by the PreCNet.
    ...
    """
    def __init__(self, stack_sizes, R_stack_sizes, Ahat_filt_sizes, R_filt_sizes, pixel_max=1.,
                 error_activation='relu', Ahat_activation='relu', LSTM_activation='relu',
                 LSTM_inner_activation='hard_sigmoid', output_mode='error', data_format=K.image_data_format(), **kwargs):
        ...
        for i in range(len(R_stack_sizes) - 1):
            assert R_stack_sizes[i] == stack_sizes[
                i + 1], "R_stack_sizes[i] must be equal stack_sizes[i+1] for i=0,1,..len(stack_sizes)-2"
